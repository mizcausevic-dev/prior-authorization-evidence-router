...
$stdout = Join-Path $env:TEMP ("prior-authorization-evidence-router-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("prior-authorization-evidence-router-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5438"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/evidence-lane"; File = "02-evidence-lane-proof.png"; Size = "1600,1320" },
        @{ Url = "http://127.0.0.1:$port/payer-rules"; File = "03-payer-rules-proof.png"; Size = "1600,1220" },
        @{ Url = "http://127.0.0.1:$port/approval-posture"; File = "04-approval-posture-proof.png"; Size = "1600,1220" }
    )
